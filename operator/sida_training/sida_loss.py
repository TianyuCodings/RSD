...
class SIDA_EDMLoss:
    ...
    def generator_share_encoder_loss(self, true_score, fake_score, images, labels=None, augment_pipe=None,alpha=1.2,tmax = 800,return_y_D=True):
        # images here is also the clean fake images.
        sigma_min = 0.002
        sigma_max = 80
        rho = 7.0
    ...
    def fakescore_discriminator_share_encoder_loss(self,fake_score, images, current_sigma=0.0, labels=None, augment_pipe=None,real_images=None,true_score=None,alpha=None):
        # real_images is the corrupted real images.
        # images here is the corrupted fake images.
        ...
        # y here is the corrupted fake images
        y, augment_labels = augment_pipe(images) if augment_pipe is not None else (images, torch.zeros(images.shape[0], 9).to(images.device))

        n = torch.randn_like(y) * torch.sqrt(sigma ** 2 - current_sigma ** 2)
        # y_fake is the predicted posterior mean by corrupted fake images
        noisy_input = y + n
        y_fake,logit_fake = fake_score(noisy_input, sigma, labels, augment_labels=augment_labels,return_flag='encoder_decoder')

        # add ambient best predicted y_0
        D_yn = ambient_utils.from_x0_pred_to_xnature_pred_ve_to_ve(y_fake, noisy_input, sigma, current_sigma)

        with torch.no_grad():
            #weight_factor = abs(y - y_fake).to(torch.float32).mean(dim=[1, 2, 3], keepdim=True).clip(min=0.00001)
            #TODO: Not sure whether this is the best way to modify the weight factor
            weight_factor = abs(y - D_yn).to(torch.float32).mean(dim=[1, 2, 3], keepdim=True).clip(min=0.00001)
        ...
        nan_mask = torch.isnan(y).flatten(start_dim=1).any(dim=1) | torch.isnan(y_fake).flatten(start_dim=1).any(dim=1) | torch.isnan(D_yn).flatten(start_dim=1).any(dim=1)
        ...
        loss_fake_score = weight * ((D_yn - y) ** 2)
