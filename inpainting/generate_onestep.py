...
@click.option('--corruption_probability',   help='Stoch. noise std', metavar='FLOAT',                                type=float, default=0.8, show_default=True)
@click.option('--delta_probability',     help='Stoch. noise std', metavar='FLOAT',                                type=float, default=0.1, show_default=True)

def main(network_pkl, outdir, subdirs, seeds, class_idx, max_batch_size, num_fid_samples, sigma_G, corruption_probability, delta_probability,  device=torch.device('cuda')):
    ...
    for batch_seeds in tqdm.tqdm(rank_batches, unit='batch', disable=(dist.get_rank() != 0)):
        ...
        latents = sigma_G * rnd.randn([batch_size, net.img_channels // 2, net.img_resolution, net.img_resolution], device=device)
        survival_probability = (1 - corruption_probability) * (1 - delta_probability)
        mask = get_sampling_random_mask(latents.shape, survival_probability, device=device)

        masked_latents = latents * mask
        G_input = torch.cat([masked_latents, mask], dim=1).to(torch.float64)
        ...
        images = net(G_input, sigma, class_labels).to(torch.float64)[:, :3]
