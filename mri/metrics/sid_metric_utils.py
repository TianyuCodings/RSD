...
class MetricOptions:
    def __init__(self, G=None, init_sigma=None, G_kwargs={}, dataset_kwargs={}, data_stat=None,detector_url=None, num_gpus=1, rank=0, local_rank=0, device=None, progress=None, cache=True, **kwargs):
...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, **stats_kwargs):
    ...
    for images, _labels, _, _, _ in torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs):
        images = images[:,0,...] + 1j*images[:,1,...]
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, jit=False, **stats_kwargs):
    ...
    dataset = training.dataset.NumpyFolderDataset(**opts.dataset_kwargs)

    # Image generation func.
    def run_generator(z, c, init_sigma):
        img = G(z, init_sigma*torch.ones(z.shape[0],1,1,1).to(z.device), c, augment_labels=torch.zeros(z.shape[0], 9).to(z.device))[:,:2]
        img = img[:,0,...] + 1j*img[:,1,...]
        img = torch.abs(img)
        img = img[:,None,...]
        return img
    ...
    # JIT.
    if jit:
        z = init_sigma*torch.zeros([batch_gen, G.img_channels//2, G.img_resolution, G.img_resolution], device=opts.device)
    ...
    dataset_sampler = misc.InfiniteSampler(dataset=dataset, rank=dist.get_rank(), num_replicas=dist.get_world_size(), seed=0)
    dataset_sampler2 = misc.InfiniteSampler(dataset=dataset, rank=dist.get_rank(), num_replicas=dist.get_world_size(), seed=1)
    data_iterator = torch.utils.data.DataLoader(dataset=dataset, sampler=dataset_sampler, batch_size=batch_gen, pin_memory=True, num_workers=3, prefetch_factor=2)
    data_iterator = iter(data_iterator)
    loss_fn = sid_loss.SID_EDMLoss(norm=2)
    data_iterator2 = torch.utils.data.DataLoader(dataset=dataset, sampler=dataset_sampler2, batch_size=batch_gen, pin_memory=True, num_workers=3, prefetch_factor=2)
    data_iterator2 = iter(data_iterator2)

    # Main loop.
    while not stats.is_full():
        images = []
        for _i in range(batch_size // batch_gen):
            z = init_sigma*torch.randn([batch_gen, G.img_channels//2, G.img_resolution, G.img_resolution], device=opts.device)
            _, _, _, _, maps = next(data_iterator)
            _, _, _, corruption_matrix, _ = next(data_iterator2)
            corruption_matrix = corruption_matrix.to(z.device)
            maps = maps.to(z.device)
            G_input = loss_fn.prepare_data(z, corruption_matrix, maps)   
            c = [dataset.get_label(np.random.randint(len(dataset))) for _i in range(batch_gen)]
            c = torch.from_numpy(np.stack(c)).pin_memory().to(opts.device)
            images.append(run_generator(G_input, c, init_sigma))
