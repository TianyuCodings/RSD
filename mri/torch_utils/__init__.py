...
def save_images(images, image_path, num_rows=None, num_cols=None, save_wandb=False, down_factor=None, wandb_down_factor=None, 
    ...
    if img_channels == 2:
        ...
        images_np = images[:,0,...].cpu().detach() + 1j*images[:,1,...].cpu().detach()
        image_np = (torch.abs(images_np)).cpu().numpy()
        grid_image = PIL.Image.new('L', (num_cols * image_size, num_rows * image_size))
        ...
        image_np = np.rint(image_np).clip(0, 255).astype(np.uint8)
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                ...
                img = PIL.Image.fromarray(image_np[index][0])
                grid_image.paste(img, (j * image_size, i * image_size))
                if captions is not None:
                    draw = PIL.ImageDraw.Draw(grid_image)
                    # use LaTeX bold font
        ...
        raise ValueError("Unsupported number of channels: {}".format(img_channels))
