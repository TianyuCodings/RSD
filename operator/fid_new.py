...
def calculate_inception_stats(
    ...
    for images, _labels in tqdm.tqdm(data_loader, unit='batch', disable=(dist.get_rank() != 0)):
        ...
        if sigma_level > 0.0:
            ...
            images += torch.randn_like(images) * sigma_level
            images = (images * 127.5 + 128).clamp(0, 255).to(images_dtype)
...
def calc(image_path, ref_path, num_expected, seed, batch):
    ...
    mu, sigma = calculate_inception_stats(image_path=image_path, num_expected=num_expected, seed=seed, max_batch_size=batch, sigma_level=0.0)
...
@click.option('--batch',                help='Maximum batch size', metavar='INT',       type=click.IntRange(min=1), default=64, show_default=True)
@click.option('--sigma_level',          help='Add noise to images', type=float, default=0.0, show_default=True)

def ref(dataset_path, dest_path, batch, sigma_level):
    """Calculate dataset reference statistics needed by 'calc'."""
    torch.multiprocessing.set_start_method('spawn')
    dist.init()

    mu, sigma = calculate_inception_stats(image_path=dataset_path, max_batch_size=batch, sigma_level=sigma_level)
