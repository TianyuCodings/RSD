...
def create_masks(R, delta_R, acs_lines, LENGTH_Y, LENGTH_X, return_original=False):
    if delta_R == 3:
        delta_R=54
    elif delta_R == 5:
        delta_R=16
    ...
    if return_original:
        return torch.tensor(mask), torch.tensor(further_mask)
    else:
        return torch.tensor(further_mask)

def create_masks_batch(batch_size, R, delta_R, acs_lines, LENGTH_Y=384, LENGTH_X=320, device='cuda'):
    if delta_R == 3:
        delta_R = 54
    elif delta_R == 5:
        delta_R = 16
    ...
    for _ in range(batch_size):
        ...
        masks.append(mask)
        further_masks.append(further_mask)


    return (
        torch.tensor(np.array(masks), dtype=torch.float32, device=device)[:, None, ...],
        torch.tensor(np.array(further_masks), dtype=torch.float32, device=device)[:, None, ...]
            )