...
def ambient_sampler(
    ...
    delta_corr = corr
    path_prefix = file.namelist()[0]
    assert path_prefix is not None, "No folders found in the zip file"
    if corr > 1:
        delta_corr = corr + 1
        for i in range(latents.shape[0]):
            with file.open(path_prefix + str(i) + "/maps.npy", 'r') as f:
                map = np.load(f)
                map = fftmod(map)
                map = torch.tensor(map, dtype=torch.complex64).to(latents.device)
                maps[i] = map
            masks[i,0] = create_masks(corr, delta_corr, 20, latents.shape[-2], latents.shape[-1], return_original=False)
