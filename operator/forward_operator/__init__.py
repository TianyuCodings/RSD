...
class DownSampling(Operator):
    def __init__(self, resolution=64, scale_factor=4, device='cuda', sigma=0.05):
        super().__init__(sigma)
...
def random_sq_bbox(img, mask_shape, image_size=64, margin=(16, 16)):
...
class Inpainting(Operator):
    def __init__(self, mask_type, mask_len_range=None, mask_prob_range=None, resolution=64, device='cuda',
                 sigma=0.05):
        super().__init__(sigma)
...
class GaussianBlur(Operator):
    def __init__(self, kernel_size, intensity, device='cuda', sigma=0.05, requires_grad=False):
        super().__init__(sigma)
        self.device = device
        self.kernel_size = kernel_size
...
class PhaseRetrieval(Operator):
    def __init__(self, oversample=0.0, resolution=64, sigma=0.05):
        super().__init__(sigma)
