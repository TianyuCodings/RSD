...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        sigma=0.0,
        noise_type="ve",
        only_positive=True,  # whether to return images in [0, 1] or [-1, 1]s
        operator='identity',
    ):
        ...
        if operator == "identity":
            ...
            self.operator = get_operator(**config)
        elif operator == "deblur2":
            config = {'name': 'deblur2', "channels": self.num_channels, "img_dim": self.resolution,  'device':"cuda:0"}
            self.operator = get_operator(**config)
        elif operator == "sr":
            config = {'name': 'down_sampling', 'scale_factor': 2, 'resolution': self.resolution, 'sigma': 0.0, 'device':"cuda:0"}
            self.operator = get_operator(**config)
        elif operator == "hdr":
            config = {'name': 'high_dynamic_range', 'scale': 2, 'sigma': 0.0, 'device':"cuda:0"}
            self.operator = get_operator(**config)
        elif operator == "phase":
            config = {'name': 'phase_retrieval', 'oversample': 2.0, 'resolution': self.resolution, 'sigma': 0.0}
            self.operator = get_operator(**config)
        elif operator == "sr2":
            config = {'name': 'down_sampling2', 'scale_factor': 2, 'resolution': self.resolution, 'sigma': 0.0, 'device':"cuda:0"}
            self.operator = get_operator(**config)
        else:
    ...
    def __getitem__(self, idx):
        ...
        if self.operator is not None:
            # Convert to PyTorch tensor in pinned memory for faster transfer
            image = torch.from_numpy(image).pin_memory()
            # Move to GPU and apply operator with non_blocking=True
            image = self.operator(image.to("cuda:0", non_blocking=True))
            # Only move back to CPU if necessary
            image = image.cpu().numpy()
        

        assert isinstance(image, np.ndarray)
        #assert list(image.shape) == self.image_shape # because of sr operator
        if self._xflip[idx]:
