...
from . import rsd_metric_utils as metric_utils
from . import rsd_frechet_inception_distance as frechet_inception_distance
from . import rsd_kernel_inception_distance
from . import rsd_precision_recall as precision_recall
from . import perceptual_path_length
from . import rsd_inception_score as inception_score
...
@register_metric
def fid01k_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fid = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=100)
    return dict(fid01k_full=fid)

@register_metric
def kid50k_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    kid = rsd_kernel_inception_distance.compute_kid(opts, max_real=1000000, num_gen=50000, num_subsets=100, max_subset_size=1000)
...
def kid50k(opts):
    opts.dataset_kwargs.update(max_size=None)
    kid = rsd_kernel_inception_distance.compute_kid(opts, max_real=50000, num_gen=50000, num_subsets=100, max_subset_size=1000)
