...
class MetricOptions:
    def __init__(self, G=None, init_sigma=None, G_kwargs={}, dataset_kwargs={}, survival_prob=0.5, data_stat=None, num_gpus=1, rank=0, local_rank=0, device=None, progress=None, cache=True):
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, jit=False, **stats_kwargs):
    ...
    def run_generator(z, c, init_sigma):
        corruption_mask = get_sampling_random_mask(z.shape, opts.survival_prob, device=opts.device)
        z_masked = corruption_mask * z
        G_input = torch.cat([z_masked, corruption_mask], dim=1)
        img = G(G_input, init_sigma*torch.ones(z.shape[0],1,1,1).to(z.device), c, augment_labels=torch.zeros(z.shape[0], 9).to(z.device))[:, :3]
